...
begin
  ...
  begin
    ...
    {$IFDEF VER5U}
    if (Comp is TToolBar) and (xcToolBar in FXPSettings.XPControls) then
        ...
        begin
          for x := 0 to TToolBar(Comp).ButtonCount - 1 do
            if (not assigned(TToolBar(Comp).OnCustomDrawButton))
              or (XPSettings.OverrideOwnerDraw) then
            begin
    ...
    if not (csDesigning in ComponentState) then
      if {$IFDEF VER6U}
        ...
        if ((TControl(Comp).Parent is TToolbar) and (xccToolBar in FXPSettings.XPContainers))or
           ...
           {$IFDEF VER5U}
           ((TControl(Comp).Parent is TCustomFrame)  and (xccFrame in FXPSettings.XPContainers)) or
           {$ENDIF}
    ...
    {$IFDEF VER5U}
    if ((Comp is TCustomFrame) and (xccFrame in XPSettings.XPContainers))
...
begin
  ...
  if (Is16Bit) and (not XPSettings.UseSystemColors) then
    FBSelectColor := NewColor(ACanvas, XPSettings.SelectColor, 68)
  else
...
//-----------
  ...
  begin
    ...
    DrawTheText(Button, Button.Caption, '',
      ...
      (Button.MenuItem <> nil),
      (Button.BidiMode = bdRightToLeft), XPSettings.Font, TextFormat);
  ...
  if Button.MenuItem <> nil then
    if (xcMainMenu in XPSettings.XPControls) then
